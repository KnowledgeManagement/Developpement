		...
		<form method="POST" id="formulaire" name="formulaire" enctype="multipart/form-data" action="Defauts\Contenu\functions\UpdateFunction.php">
			...
			<table class="table" style="width : 100%;">
				...
				<tr>
					...
					<td><textarea style="min-height:130px;resize:vertical;" class="form-control" name="description"><?php echo $myFunction[0]['description']; ?></textarea></td>
				</tr>
				...
				<tr>
					...
					<td>
						<?php
							...
							echo '<textarea id="explication'.$exp.'" style="min-height:130px;resize:vertical;" class="form-control" name="explication'.$exp.'" required>'.$explodeFinalExpli[0].'</textarea>';
							$exp++;
						?>
						...
						<?php
							if(isset($explodeSection[1])){
								for($i = 1; $i < sizeof($explodeSection); $i++){
									...
									echo '</br><span class="alert alert-danger pull-left"><i class="glyphicon glyphicon-exclamation-sign"></i> Champ exclusivement réservé au code.</span></br><textarea id="exemple'.$ex.'" style="min-height:130px;resize:vertical;" class="form-control" name="exemple'.$ex.'" required>'.$codeFinal[0].'</textarea>';
									$ex++;
				...
				<tr>
					...
					?>
						...
						<td><input style="width: 145px;" type="file" name="pj" id="piecejointe" value="<?php echo $uploaddir; ?>"/><label> <?php echo $link; ?> </label></td>
				</tr>
